...
def do_train(
    ...
    for epoch in range(0, max_epoch):
        ...
        for iteration, (batch_img, batch_att, batch_label) in enumerate(tr_dataloader):
            ...
            loss = Lcls + lamd[1]*Lreg +Lbias
            #print(lamd[1],lamd[2],lamd[3])
            ...
            losses_reduced = lcls + lamd[1]*lreg + lbias
        ...
        if is_main_process():
            ...
            log_info = 'epoch: %d  |  loss: %.4f (%.4f), cls_loss: %.4f (%.4f), reg_loss: %.4f (%.4f),  bias_loss: %.4f (%.4f),   scale:  %.4f (%.4f),    lr: %.6f' % \
                       (epoch + 1, loss_epoch_mean, losses_mean, cls_loss_epoch_mean, cls_losses_mean,reg_loss_epoch_mean,
                        reg_losses_mean, bias_loss_epoch_mean, bias_losses_mean,
