...
def cal_accuracy(model, dataloadr, att, test_id, device,att_all, bias=None):
    ...
    '''
    for i1 in unique_pre:
        if i1 not in unique_labels:
            for i2 in range(len(outpred)):
                if outpred[i2] == i1:
                    sum1 += 1
    '''
    for i1 in range(len(outpred)):
        if labels[i1] != outpred[i1]:
            sum_wrong_all += 1
            if outpred[i1] not in unique_labels:
                sum_wrong_c += 1
    print(sum_wrong_all,sum_wrong_c,len(outpred))
